...
export default function PlaceCard({ place, distanceInKm, onClick }) {
  ...
  const imageSrc =
    !imgErr && place.images?.[0] ? place.images[0] : FALLBACK_IMG;
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
        ...
        {place.entryfees != null && (
          <span className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-gray-800
        ...
        {/* Distance badge */}
        {distLabel && (
          <span className="absolute bottom-3 right-3 flex items-center gap-1 bg-black/70
        ...
        {/* Popular badge */}
        {place.isPopular && (
          <span className="absolute bottom-3 left-3 bg-amber-400 text-amber-900
                           text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
            ⭐ Popular
          </span>
        )}
      ...
      <div className="flex flex-col gap-2 p-4 flex-1">
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-50">
          ...
          <button className="ml-auto text-xs font-semibold text-rose-500 hover:text-rose-600
